...
const forgetPassword = async (req, res, { userModel }) => {
  ...
  const invenstock_app_email = settings['invenstock_app_email'];
  const invenstock_base_url = settings['invenstock_base_url'];

  const url = checkAndCorrectURL(invenstock_base_url);
  ...
  await sendMail({
    ...
    name: user.name,
    link,
    subject: 'Reset your password | invenstock',
    invenstocktocktock_app_email,
