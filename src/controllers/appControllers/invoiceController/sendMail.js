...
const sendViaApi = async ({ email, name, targetLocation }) => {
  try {
    ...
    const invenstock_app_email = 'noreply@invenstockapp.com';
    const invenstock_app_company_email = settings['invenstock_app_company_email'];
    ...
    const { data } = await resend.emails.send({
      from: invenstock_app_email,
      to: email,
      subject: 'Invoice From ' + company_name,
      reply_to: invenstock_app_company_email,
      attachments: [
        {
